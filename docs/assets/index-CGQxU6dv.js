...
(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const p of a.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&r(p)}).observe(document,{childList:!0,subtree:!0});function l(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(n){if(n.ep)return;n.ep=!0;const a=l(n);fetch(n.href,a)}})();const B=[];let S=null;function $(e,t){B.push({pattern:e,handler:t})}function V(e){const t=e.replace(/^#\/?/,"/");for(const{pattern:l,handler:r}of B){const n=Y(l,t);if(n!==null)return{handler:r,params:n}}return null}function Y(e,t){const l=e.split("/").filter(Boolean),r=t.split("/").filter(Boolean);if(l.length!==r.length)return null;const n={};for(let a=0;a<l.length;a++)if(l[a].startsWith(":"))n[l[a].slice(1)]=decodeURIComponent(r[a]);else if(l[a]!==r[a])return null;return n}function Z(e){async function t(){const l=window.location.hash||"#/",r=V(l);S&&(S(),S=null),r?(e.innerHTML="",S=await r.handler(e,r.params)||null):window.location.hash="#/",document.querySelectorAll(".site-header__link").forEach(n=>{const a=n.getAttribute("href");n.classList.toggle("site-header__link--active",a===l||l==="#/"&&a==="#/")})}window.addEventListener("hashchange",t),t()}function K(){const e=document.createElement("header");e.className="site-header",e.innerHTML=`
        ...
        <div class="landing__era-list">
          ${l.eras.map(r=>`
            <div class="landing__era">
              <span class="landing__era-name">${r.display}</span>
              <span class="landing__era-count">${r.count} texts</span>
            </div>
          `).join("")}
        </div>
      </section>
  ...
  `}async function Q(e){const{texts:t,facets:l}=await k(),r={};for(const n of l.eras)r[n.id]={display:n.display,texts:t.filter(a=>a.era===n.id)};e.innerHTML=`
    <div class="page syllabus">
      ...
      ${l.eras.map(n=>{const a=r[n.id].texts,p={};for(const i of a){const d=i.topics[0]||"other";p[d]||(p[d]=[]),p[d].push(i)}return`
  ...
  `,e.querySelectorAll(".syllabus__era-toggle").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.era,i=document.getElementById(`era-${a}`).classList.toggle("syllabus__era-content--open");n.querySelector(".syllabus__era-chevron").textContent=i?"▴":"▾"})})}function X(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}const ee="https://raw.githubusercontent.com/hungryrobot1/Enchiridion/main",te=location.hostname==="localhost"||location.hostname==="127.0.0.1";function T(e){const l=e.split("/").map(r=>encodeURIComponent(r)).join("/");return te?`/${l}`:`${ee}/${l}`}let m={query:"",era:"",topic:"",format:"",sort:"chronological"};async function ae(e){const{texts:t,facets:l}=await k();e.innerHTML=`
  ...
  `;const r=e.querySelector(".explorer__search-input"),n=e.querySelector(".explorer__grid"),a=e.querySelector(".explorer__results-count"),p=e.querySelectorAll(".explorer__filter-select");function i(){let s=t;if(m.query){const o=m.query.toLowerCase();s=s.filter(c=>c.title.toLowerCase().includes(o)||c.author.toLowerCase().includes(o)||c.description.toLowerCase().includes(o)||c.topics.some(u=>u.toLowerCase().includes(o)))}switch(m.era&&(s=s.filter(o=>o.era===m.era)),m.topic&&(s=s.filter(o=>o.topics.includes(m.topic))),m.format&&(s=s.filter(o=>o.format===m.format)),s=[...s],m.sort){case"reverse-chrono":s.sort((o,c)=>c.year_sort-o.year_sort);break;case"title":s.sort((o,c)=>o.title.localeCompare(c.title));break;case"author":s.sort((o,c)=>o.author.localeCompare(c.author));break}if(a.textContent=`${s.length} of ${t.length} texts`,s.length===0){n.innerHTML='<div class="explorer__empty">No texts match your filters.</div>';return}n.innerHTML=s.map(o=>`
      <a href="#/read/${o.era_dir}/${o.id}" class="text-card" data-id="${o.id}">
        ...
        <div class="text-card__footer">
          ${o.topics.slice(0,3).map(c=>`<span class="topic-pill">${M(c)}</span>`).join("")}
          <button class="text-card__download" data-path="${o.path}" data-filename="${o.filename}" title="Download">
            &#8595; Download
          </button>
        </div>
      </a>
    `).join(""),n.querySelectorAll(".text-card__download").forEach(o=>{o.addEventListener("click",c=>{c.preventDefault(),c.stopPropagation();const u=T(o.dataset.path),_=document.createElement("a");_.href=u,_.download=o.dataset.filename,_.click()})})}let d;r.addEventListener("input",()=>{clearTimeout(d),d=setTimeout(()=>{m.query=r.value,i()},200)}),p.forEach(s=>{s.addEventListener("change",()=>{const o=s.dataset.filter;o==="sort"?m.sort=s.value:m[o]=s.value,i()})}),i(),r.focus()}function M(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}const re="modulepreload",se=function(e){return"/Enchiridion/"+e},D={},y=function(t,l,r){let n=Promise.resolve();if(l&&l.length>0){let p=function(s){return Promise.all(s.map(o=>Promise.resolve(o).then(c=>({status:"fulfilled",value:c}),c=>({status:"rejected",reason:c}))))};document.getElementsByTagName("link");const i=document.querySelector("meta[property=csp-nonce]"),d=(i==null?void 0:i.nonce)||(i==null?void 0:i.getAttribute("nonce"));n=p(l.map(s=>{if(s=se(s),s in D)return;D[s]=!0;const o=s.endsWith(".css"),c=o?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${s}"]${c}`))return;const u=document.createElement("link");if(u.rel=o?"stylesheet":re,o||(u.as="script"),u.crossOrigin="",u.href=s,d&&u.setAttribute("nonce",d),document.head.appendChild(u),o)return new Promise((_,f)=>{u.addEventListener("load",_),u.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${s}`)))})}))}function a(p){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=p,window.dispatchEvent(i),!i.defaultPrevented)throw p}return n.then(p=>{for(const i of p||[])i.status==="rejected"&&a(i.reason);return t().catch(a)})};let x=null;async function j(){if(x)return x;const t=await fetch("/Enchiridion/supplement-index.json");return t.ok?(x=await t.json(),x):(x={supplements:[],facets:{eras:[],types:[]}},x)}function C(e){const t=e.querySelector(".reader__fullscreen");if(!t)return;const l=document.fullscreenEnabled;t.addEventListener("click",()=>{if(l)document.fullscreenElement?document.exitFullscreen().catch(()=>{}):document.documentElement.requestFullscreen().catch(()=>{});else{document.documentElement.classList.toggle("focus-mode");const r=document.documentElement.classList.contains("focus-mode");t.textContent=r?"×":"⛶",t.title=r?"Exit fullscreen":"Toggle fullscreen"}}),l&&document.addEventListener("fullscreenchange",()=>{t.textContent=document.fullscreenElement?"×":"⛶",t.title=document.fullscreenElement?"Exit fullscreen":"Toggle fullscreen"})}const H="enchiridion-bookmarks";function F(){try{return JSON.parse(localStorage.getItem(H))||{}}catch{return{}}}function ne(e,t){const l=F();l[e]=t,localStorage.setItem(H,JSON.stringify(l))}function le(e){return F()[e]||null}async function oe(e,{era:t,id:l}){const[{texts:r},{supplements:n}]=await Promise.all([k(),j()]),a=r.find(u=>u.id===l);if(!a){e.innerHTML=`
      <div class="reader">
        ...
        <aside class="reader__sidebar">
          ...
          <div class="reader__meta-field">
            ...
            <div class="reader__meta-topics">
              ${a.topics.map(u=>`<span class="topic-pill">${de(u)}</span>`).join("")}
            </div>
          </div>
          ${a.description?`
            <div class="reader__meta-field">
          ...
          ${a.prerequisites.length>0?`
            <div class="reader__meta-field">
              ...
              ${a.prerequisites.map(u=>{const _=r.find(f=>f.id===u);return _?`<a href="#/read/${_.era_dir}/${_.id}" class="reader__meta-prereq">${_.title}</a>`:`<span class="reader__meta-value">${u}</span>`}).join("")}
            </div>
          `:""}
          ${(()=>{const u=n.filter(_=>_.texts.includes(a.id));return u.length===0?"":`
              <div class="reader__meta-field">
  ...
  `,e.querySelector(".reader__download").addEventListener("click",()=>{const u=document.createElement("a");u.href=p,u.download=a.filename,u.click()}),C(e);const d=e.querySelector(".reader__sidebar"),s=e.querySelector(".reader__sidebar-toggle");d.classList.add("reader__sidebar--collapsed"),s.addEventListener("click",()=>{const u=d.classList.toggle("reader__sidebar--collapsed");s.textContent=u?"Show Details":"Hide Details"});const o=e.querySelector(".reader__viewport-inner");let c=null;try{const u=await ie(a.format);if(i){const _=e.querySelector(".reader__page-input"),f=e.querySelector(".reader__page-total"),h=e.querySelector(".reader__zoom-in"),v=e.querySelector(".reader__zoom-out"),L=e.querySelector(".reader__zoom-level");let A=null;c=await u.render(o,p,e,{onReady:b=>{h.addEventListener("click",()=>b.zoomIn()),v.addEventListener("click",()=>b.zoomOut()),_.addEventListener("keydown",g=>{if(g.key==="Enter"){const P=parseInt(_.value,10);isNaN(P)||b.goToPage(P),_.blur()}}),_.addEventListener("blur",()=>{const g=parseInt(_.value,10);isNaN(g)||b.goToPage(g)}),_.addEventListener("focus",()=>_.select());const w=le(l);if(w&&w>1){const g=document.createElement("div");g.className="reader__resume-banner",g.innerHTML=`
            ...
            `,e.querySelector(".reader__toolbar").after(g),g.querySelector('[data-action="resume"]').addEventListener("click",()=>{b.goToPage(w),g.remove()}),g.querySelector('[data-action="dismiss"]').addEventListener("click",()=>{g.remove()})}A=setInterval(()=>{const g=b.getCurrentPage();g>1&&ne(l,g)},5e3)},onPageChange:(b,w)=>{_.value=b,_.style.width=`${String(w).length+1}ch`,f.textContent=`of ${w}`},onScaleChange:b=>{L.textContent=`${Math.round(b*50)}%`,h.disabled=b>=4,v.disabled=b<=1}});const I=c;c=()=>{A&&clearInterval(A),I&&I()}}else c=await u.render(o,p,e)}catch(u){console.error("Reader error:",u),o.innerHTML=`
      <div class="reader__error">
    ...
    `}return()=>{c&&c()}}async function ie(e){switch(e){case"epub":return(await y(async()=>{const{default:t}=await import("./epub-reader-i1hTWVgL.js");return{default:t}},[])).default;case"pdf":return(await y(async()=>{const{default:t}=await import("./pdf-reader-DEUgGmV0.js");return{default:t}},[])).default;case"html":return(await y(async()=>{const{default:t}=await import("./html-reader-BvnsfJ7c.js");return{default:t}},[])).default;case"txt":return(await y(async()=>{const{default:t}=await import("./txt-reader-DQg-AX_E.js");return{default:t}},[])).default;case"md":case"markdown":return(await y(async()=>{const{default:t}=await import("./md-reader-C6phV3Oz.js");return{default:t}},__vite__mapDeps([0,1]))).default;default:throw new Error(`Unsupported format: ${e}`)}}function de(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}const U={"exercise-set":"Exercise Sets","lab-manual":"Lab Manuals","notation-guide":"Notation Guides","convention-guide":"Convention Guides","study-guide":"Study Guides"},O=Object.keys(U);async function ce(e){const{supplements:t,facets:l}=await j();if(t.length===0){e.innerHTML=`
    ...
    `;return}const r=t.filter(i=>i.type!=="reference"),n={};for(const i of l.eras)n[i.id]={display:i.display,count:r.filter(d=>d.era===i.id).length,supplements:r.filter(d=>d.era===i.id)};const a=l.eras.filter(i=>r.some(d=>d.era===i.id)).length,p=r.length;e.innerHTML=`
    <div class="page supplements">
      <header class="supplements__header">
        <h1>Supplements</h1>
      ...
      ${l.eras.map(i=>{const d=n[i.id];if(!d||d.supplements.length===0)return"";const s={};for(const c of d.supplements){const u=c.type||"other";s[u]||(s[u]=[]),s[u].push(c)}const o=Object.entries(s).sort((c,u)=>{const _=O.indexOf(c[0]),f=O.indexOf(u[0]);return(_>=0?_:999)-(f>=0?f:999)});return`
  ...
  `,e.querySelectorAll(".supplements__era-toggle").forEach(i=>{i.addEventListener("click",()=>{const d=i.dataset.era,o=document.getElementById(`sup-era-${d}`).classList.toggle("supplements__era-content--open");i.querySelector(".supplements__era-chevron").textContent=o?"▴":"▾"})})}function ue(e){return e.split("-").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}const pe={"exercise-set":"Exercise Set","lab-manual":"Lab Manual","notation-guide":"Notation Guide","convention-guide":"Convention Guide",reference:"Reference"};async function _e(e,{era:t,id:l}){const[{supplements:r},{texts:n}]=await Promise.all([j(),k()]),a=r.find(h=>h.id===l);if(!a){e.innerHTML=`
      <div class="reader">
        <div class="reader__error">
    ...
    `;return}const p=T(a.path),i=a.format||"md",d=a.texts.map(h=>n.find(v=>v.id===h)).filter(Boolean),s=a.type==="reference"?"Topic":"Era",o=a.era_display;e.innerHTML=`
    <div class="reader">
      <div class="reader__toolbar">
        <button class="reader__back" onclick="history.back()">&larr; Back</button>
        <span class="reader__toolbar-title">${a.title}</span>
      ...
      <div class="reader__body">
        <aside class="reader__sidebar">
          ...
          <div class="reader__meta-field">
            ...
            <span class="supplements__type-badge">${pe[a.type]||a.type}</span>
          </div>
          <div class="reader__meta-field">
          ...
          ${a.prerequisites.length>0?`
            <div class="reader__meta-field">
              <span class="reader__meta-label">Prerequisites</span>
              ${a.prerequisites.map(h=>{const v=r.find(L=>L.id===h);return v?`<a href="#/supplement/${encodeURIComponent(v.era_dir)}/${v.id}" class="reader__meta-prereq">${v.title}</a>`:`<span class="reader__meta-value">${h}</span>`}).join("")}
            </div>
          `:""}
        </aside>
        <div class="reader__viewport">
  ...
  `,e.querySelector(".reader__download").addEventListener("click",()=>{const h=document.createElement("a");h.href=p,h.download=a.path.split("/").pop(),h.click()});const c=e.querySelector(".reader__sidebar"),u=e.querySelector(".reader__sidebar-toggle");c.classList.add("reader__sidebar--collapsed"),u.addEventListener("click",()=>{const h=c.classList.toggle("reader__sidebar--collapsed");u.textContent=h?"Show Details":"Hide Details"}),C(e);const _=e.querySelector(".reader__viewport-inner");let f=null;try{f=await(await he(i)).render(_,p,e)}catch(h){console.error("Supplement reader error:",h),_.innerHTML=`
    ...
    `}return()=>{f&&f()}}async function he(e){switch(e){case"epub":return(await y(async()=>{const{default:t}=await import("./epub-reader-i1hTWVgL.js");return{default:t}},[])).default;case"pdf":return(await y(async()=>{const{default:t}=await import("./pdf-reader-DEUgGmV0.js");return{default:t}},[])).default;case"html":return(await y(async()=>{const{default:t}=await import("./html-reader-BvnsfJ7c.js");return{default:t}},[])).default;case"txt":return(await y(async()=>{const{default:t}=await import("./txt-reader-DQg-AX_E.js");return{default:t}},[])).default;case"md":default:return(await y(async()=>{const{default:t}=await import("./md-reader-C6phV3Oz.js");return{default:t}},__vite__mapDeps([0,1]))).default}}let E=null;async function z(){if(E)return E;const t=await fetch("/Enchiridion/module-index.json");return t.ok?(E=await t.json(),E):(E={modules:[]},E)}function me(e){return!e||e.length===0?"":e.map(t=>t.split("-").map(l=>l.charAt(0).toUpperCase()+l.slice(1)).join(" ")).join(", ")}async function fe(e){const{modules:t}=await z();if(t.length===0){e.innerHTML=`
    ...
    `;return}const l=t.reduce((r,n)=>r+n.chapters.length,0);e.innerHTML=`
    <div class="page modules">
      <header class="modules__header">
        <h1>Modules</h1>
      ...
      ${t.map(r=>{const n=r.chapters.length,a=r.prerequisites.length>0,p=r.resources.length>0,i=r.references.length>0;return`
          <section class="modules__module">
            <button class="modules__toggle" data-module="${r.id}">
              <h2>${r.title}</h2>
              <span class="modules__count">${n} chapter${n!==1?"s":""}</span>
              <span class="modules__chevron">&#9662;</span>
            </button>
            <div class="modules__content" id="mod-${r.id}">
              <p class="modules__description">${r.description}</p>

              ${a?`
                <p class="modules__prerequisites">
                  Prerequisites: ${r.prerequisites.map(d=>{const s=t.find(o=>o.id===d);return s?`<a href="#/modules" data-prereq="${d}">${s.title}</a>`:d}).join(", ")}
                </p>
              `:""}

              <h3 class="modules__section-heading">Chapters</h3>
              <ul class="modules__chapters">
                ${r.chapters.map((d,s)=>{const o=me(d.alongside);return`
                    <li>
                      <a href="#/module/${r.id}/${d.filename}" class="modules__chapter-link">
              ...
              ${p?`
                ...
                <ul class="modules__resources">
                  ${r.resources.map(d=>`
                    <li>
                      <a href="#/module/${r.id}/resource/${d.filename}" class="modules__resource-link">${d.title}</a>
                    </li>
                  `).join("")}
                </ul>
              `:""}
              ...
              ${i?`
                ...
                <ul class="modules__references">
                  ${r.references.map(d=>{const s=d.url&&!d.path;return`
                      <li>
                        <a href="${s?d.url:d.path?`#/supplement/${encodeURIComponent(d.era_dir)}/${d.id}`:d.url||"#"}"${s?' target="_blank" rel="noopener noreferrer"':""} class="modules__ref-link">
  ...
  `,e.querySelectorAll(".modules__toggle").forEach(r=>{r.addEventListener("click",()=>{const n=r.dataset.module,p=document.getElementById(`mod-${n}`).classList.toggle("modules__content--open");r.querySelector(".modules__chevron").textContent=p?"▴":"▾"})})}async function G(e,{id:t,chapter:l}){const[{modules:r},{texts:n}]=await Promise.all([z(),k()]),a=r.find(d=>d.id===t);if(!a){e.innerHTML=`
      <div class="reader">
        <div class="reader__error">
    ...
    `;return}const p=l.startsWith("resource/"),i=p?l.replace("resource/",""):l;return p?ge(e,a,i):ve(e,a,i,n)}async function ve(e,t,l,r){const n=t.chapters.findIndex(h=>h.filename===l);if(n===-1){e.innerHTML=`
      <div class="reader">
        <div class="reader__error">
    ...
    `;return}const a=t.chapters[n],p=`supplements/modules/${t.id}/${l}`,i=T(p),d=n>0?t.chapters[n-1]:null,s=n<t.chapters.length-1?t.chapters[n+1]:null,o=(a.alongside||[]).map(h=>r.find(v=>v.id===h)).filter(Boolean);e.innerHTML=`
    <div class="reader">
      <div class="reader__toolbar">
      ...
      <div class="reader__body">
        <aside class="reader__sidebar">
          ...
          <div class="reader__meta-field">
            ...
            <span class="reader__meta-value">${n} of ${t.chapters.length-1}</span>
          </div>
          ${o.length>0?`
            <div class="reader__meta-field">
  ...
  `,e.querySelector(".reader__back").addEventListener("click",()=>history.back()),e.querySelector(".reader__download").addEventListener("click",()=>{const h=document.createElement("a");h.href=i,h.download=l,h.click()});const c=e.querySelector(".reader__sidebar"),u=e.querySelector(".reader__sidebar-toggle");c.classList.add("reader__sidebar--collapsed"),u.addEventListener("click",()=>{const h=c.classList.toggle("reader__sidebar--collapsed");u.textContent=h?"Show Details":"Hide Details"}),C(e);const _=e.querySelector(".reader__viewport-inner");let f=null;try{f=await(await y(async()=>{const{default:L}=await import("./md-reader-C6phV3Oz.js");return{default:L}},__vite__mapDeps([0,1]))).default.render(_,i,e),N(_,t);const v=document.createElement("div");v.className="module-reader__nav",v.innerHTML=`
      ...
      <a href="${s?`#/module/${t.id}/${s.filename}`:"#"}"
         class="module-reader__nav-btn ${s?"":"module-reader__nav-btn--disabled"}"
         style="text-align: right">
        <span class="module-reader__nav-label">Next &rarr;</span>
        <span class="module-reader__nav-title">${s?s.title:""}</span>
      </a>
    ...
    `}return()=>{f&&f()}}async function ge(e,t,l){const r=t.resources.find(c=>c.filename===l),n=r?r.title:l,a=`supplements/modules/${t.id}/${l}`,p=T(a);e.innerHTML=`
    <div class="reader">
      <div class="reader__toolbar">
        <button class="reader__back">&larr; Back</button>
        <span class="reader__toolbar-title">${n}</span>
        <div class="reader__toolbar-controls">
          <button class="btn reader__download" title="Download">&#8595; Download</button>
          <button class="reader__tool-btn reader__fullscreen" title="Toggle fullscreen" aria-label="Toggle fullscreen">&#x26F6;</button>
        </div>
  ...
  `,e.querySelector(".reader__back").addEventListener("click",()=>history.back()),e.querySelector(".reader__download").addEventListener("click",()=>{const c=document.createElement("a");c.href=p,c.download=l,c.click()});const i=e.querySelector(".reader__sidebar"),d=e.querySelector(".reader__sidebar-toggle");i.classList.add("reader__sidebar--collapsed"),d.addEventListener("click",()=>{const c=i.classList.toggle("reader__sidebar--collapsed");d.textContent=c?"Show Details":"Hide Details"}),C(e);const s=e.querySelector(".reader__viewport-inner");let o=null;try{o=await(await y(async()=>{const{default:u}=await import("./md-reader-C6phV3Oz.js");return{default:u}},__vite__mapDeps([0,1]))).default.render(s,p,e),N(s,t)}catch(c){console.error("Resource reader error:",c),s.innerHTML=`
      <div class="reader__error">
        <p>Failed to load resource.</p>
        <p style="font-size: var(--text-xs); color: var(--color-text-muted);">${c.message}</p>
        <a href="${p}" class="btn" target="_blank" rel="noopener">Open Raw File</a>
      </div>
    `}return()=>{o&&o()}}function N(e,t){e.querySelectorAll("a[href]").forEach(l=>{const r=l.getAttribute("href");if(!r||r.startsWith("http")||r.startsWith("#")||!r.endsWith(".md"))return;const n=r.split("/").pop();if(t.chapters.some(a=>a.filename===n)){l.setAttribute("href",`#/module/${t.id}/${n}`);return}if(t.resources&&t.resources.some(a=>a.filename===n)){l.setAttribute("href",`#/module/${t.id}/resource/${n}`);return}})}function be(e){e.innerHTML=`
  ...
  `}const W=document.getElementById("app");W.appendChild(K());const R=document.createElement("main");R.id="content";W.appendChild(R);$("/",e=>J(e));$("/syllabus",e=>Q(e));$("/explore",e=>ae(e));$("/read/:era/:id",(e,t)=>oe(e,t));$("/supplements",e=>ce(e));$("/supplement/:era/:id",(e,t)=>_e(e,t));$("/modules",e=>fe(e));$("/module/:id/:chapter",(e,t)=>G(e,t));$("/module/:id/resource/:filename",(e,t)=>G(e,{id:t.id,chapter:`resource/${t.filename}`}));$("/disclaimer",e=>be(e));Z(R);"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/Enchiridion/sw.js").catch(()=>{})});
