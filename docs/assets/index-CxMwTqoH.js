(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const d of n.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&o(d)}).observe(document,{childList:!0,subtree:!0});function l(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(r){if(r.ep)return;r.ep=!0;const n=l(r);fetch(r.href,n)}})();const w=[];let f=null;function g(t,s){w.push({pattern:t,handler:s})}function S(t){const s=t.replace(/^#\/?/,"/");for(const{pattern:l,handler:o}of w){const r=q(l,s);if(r!==null)return{handler:o,params:r}}return null}function q(t,s){const l=t.split("/").filter(Boolean),o=s.split("/").filter(Boolean);if(l.length!==o.length)return null;const r={};for(let n=0;n<l.length;n++)if(l[n].startsWith(":"))r[l[n].slice(1)]=decodeURIComponent(o[n]);else if(l[n]!==o[n])return null;return r}function C(t){async function s(){const l=window.location.hash||"#/",o=S(l);f&&(f(),f=null),o?(t.innerHTML="",f=await o.handler(t,o.params)||null):window.location.hash="#/",document.querySelectorAll(".site-header__link").forEach(r=>{const n=r.getAttribute("href");r.classList.toggle("site-header__link--active",n===l||l==="#/"&&n==="#/")})}window.addEventListener("hashchange",s),s()}function j(){const t=document.createElement("header");return t.className="site-header",t.innerHTML=`
  ...
  `,t}let m=null;async function y(){return m||(m=await(await fetch("/Enchiridion/text-index.json")).json(),m)}async function A(t){const{texts:s,facets:l}=await y();t.innerHTML=`
    <div class="landing">
