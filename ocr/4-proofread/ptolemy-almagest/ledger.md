...
`ocr/4-proofread/compare-runs.py`. The zodiac was chosen for this because it is
...
## Book I — the recommended-passage pass, 2026-07-31

Book I is the whole of Ptolemy's assigned reading, so it is the venue for the
recommended-passage proofreading pass. Syllabus entries rewritten from
`"Book I introduction"` / `"Book I chapters on the celestial sphere"` — which
addressed nothing — to Ptolemy's own chapter divisions, 1–16 entire.

### Chapter 15's heading is missing

The printed table of contents lists sixteen chapters for Book I. The text carries
fifteen headings: **there is no `## 15.`**, and chapter 14 ends with Ptolemy
saying "we shall … set out a table … The table is as follows," followed by no
table. A `## TABLE OF INCLINATION` appears afterwards, past chapter 16's opening
prose.

**This is a missing heading, not a displaced table.** Toomer prints a
table-chapter out of sequence when the chapter *is* a table (user), so the
ordering in the markdown is faithful to the edition and nothing should be moved.
Only the `## 15. [Table of inclination]` heading needs restoring, and its exact
form wants the page.

**Book I's contents as the removed ToC printed them** — recorded here because the
ToC section itself has been deleted from the text (a more accurate one is now
generated by the reader from the headings, and a printed ToC is editorial
apparatus under the standing policy):

```
 1. Preface                          9. On the individual concepts
 2. On the order of the theorems    10. On the size of chords
 3. That the heavens move like      11. Table of chords
    a sphere                        12. On the arc between the solstices
 4. That the earth, too, taken as   13. Preliminaries for spherical proofs
    a whole, is sensibly spherical  14. On the arcs between the equator
 5. That the earth is in the            [and the ecliptic]
    middle of the heavens           15. Table of inclination
 6. That the earth has the ratio    16. On rising-times at *sphaera recta*
    of a point to the heavens
 7. That the earth does not have
    any motion from place to place
 8. That there are two different
...
and should not be spent on workers:

- **Ch 11, the Table of Chords** (pp. 69–71) — already verified by computation
  against `Crd(θ) = 120·sin(θ/2)`: 270 entries checked, 90 arc labels restored,
  180 already correct, 0 unexplained (`6efd1e6`). A reader cannot beat that.
- **Ch 15, the Table of Inclination** (pp. 83–84) — same shape, and the same
  arithmetic check applies.
