...
Most time went into source anatomy and building an asserted HTML conversion:
the genuinely intricate parts were separating 359 very long Hoover notes from
Agricola without losing calls, preserving table geometry, and independently
reconciling a million-character result. A smaller but avoidable share went into
discovering that the recon asset census ignores linked full-size images and
that the generic plate-sequence heuristic was reporting prose coincidences.
