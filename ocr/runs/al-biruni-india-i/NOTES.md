...
- Dispatched OCR cannot work by architecture because DNS is disabled. Stage 2
  should describe paid OCR as a host handoff, not a command a dispatched worker
  can execute. The failed in-sandbox call established this gap without falling
  back to the rejected embedded layer.
- The broad margin matcher was useful but a clean result from its two-word
  threshold was not completeness evidence. One-word controls and physical strip
  review exposed 68 residual labels. This is another concrete example of why a
  probe's zero requires a demonstrated positive case.
- `strip_page_furniture.py`'s general newline collapse makes an empty page's two
  adjacent separators no longer independently splittable. Page-local work was
  therefore completed before that step. A general page model should preserve
  explicit page identity until all page-keyed edits finish.
- The generic paragraph rejoiner treats numbered prose as a possible list
  boundary. This volume contains both real numbered lists and a numbered item
  split mid-sentence, so the custom script protects a numbered opener but not a
  numbered paragraph already in progress.
- Recon's `get_image_rects()` census was pathologically slow on this full-page
  image scan; an xref-only census completed quickly. The expensive placement
  check should be sampled or skipped for scan-shaped PDFs.

## Where the time went

The genuinely intricate work was distinguishing body text from hundreds of
small marginal labels and checking the exceptions against photographed margins.
The 408-page Tesseract margin census took several minutes but converted a
line-by-line ambiguity into page-keyed evidence. Reviewing short residuals and
embedded labels was slower because it required judgment. Recon image placement
and the sandbox OCR attempt were tooling/architecture friction. Once the
inventories were frozen, rebuilding and running all acceptance checks took
about a second.
