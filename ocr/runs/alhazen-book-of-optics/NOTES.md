...
The scan is a real printed witness, but only the leaves needed for boundaries,
malformed spreads, figures/labels, embedded folio cases, and exact repairs were
read closely. The remainder has not been systematically compared with the scan.
Two extracted renderings from the same OCR request, or the scan's ABBYY text
layer and the photographed page carrying it, are not independent witnesses.

The genuinely intricate work was diagnosing and reconstructing the nine
interleaved facing-page tables and distinguishing translation content from
folio/commentary apparatus. Figure extraction itself was quick once the
whole-volume recon count was scoped correctly. Tooling made two parts harder
than necessary: the initial quadratic duplicate comparison and the triad's
inability to notice reader-incompatible `\\(...\\)` math delimiters. OCR service
time was dominated by four transient full-document 500/503 responses rather
than by a problem in the prepared PDF.
