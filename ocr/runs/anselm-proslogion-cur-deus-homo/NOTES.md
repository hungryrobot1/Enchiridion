...
- Slow because of tooling: the recon image census and the need for a host-side
  OCR relay after the sandbox DNS failure.
- Slow because the work is genuinely intricate: classifying 145 page turns,
  reading the ambiguous punctuation-ending joins, separating editorial
  apparatus from work-specific prefaces, and checking OCR outliers against
  printed leaves.
- Cheap and mechanical once encoded: regeneration, exact-count assertions,
  chapter validation, controlled cleaners, and the controlled diagnostic
  triad.
