...
The deterministic extraction itself took only a few seconds. The duplicate-page
fuzzy audit took roughly 12--15 seconds per build because no stage-1 tool exists
for the documented procedure. Most work went into source-role diagnosis,
geometry inspection, apparatus classification, and designing assertions. That
is genuinely evidentiary work, though a standard tool for derivative-PDF source
identity and duplicate probing would make it faster and harder to skip.

The stage documents were clear about test limitations. What they did not settle
was how to treat a clean born-digital PDF that is itself an unverified e-text,
or how far stage 3 may claim reader-ready paragraphs when the continuous-source
"paragraph-break oracle" does not exist.
