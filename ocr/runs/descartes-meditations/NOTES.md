# Descartes Meditations pipeline notes
...
The final candidate returns zero for all three diagnostics. That clean result
is uninformative about this prose transcription: there are zero math blocks and
zero LaTeX-bearing lines.

Before recording the negatives, retained positive controls under
`verify-controls/` proved
that each checker can see its intended defect:

- `lint-math.py`: unmatched `$`, exit 1.
- `check-math.js`: undefined KaTeX command, one of one blocks failed, exit 1.
- `check-raw-latex.js`: leaked `\\therefore`, one surviving backslash, exit 1.

The controls validate the probes, but they do not make the triad relevant to a
notation-free work or say anything about whether the words are correct.
...
## Documentation/tooling lessons and time

- `1-prepare/STAGE.md` needs cropbox-aware rendering instructions.
- The top-level README says this track was proven on "the Meditations," but no
  Descartes-specific partitioner existed in the readable repository.
- The flat extractor discards italics. Here the best method is EPUB structure
  plus exact PDF-stream equality; that mixed-source pattern deserves explicit
  documentation.
- The repository is read-only, so the new per-text tool could not be registered
  in a repository-owned stage table. It is retained in this workspace and named
  in the proposal.
- Recon, extraction, and all equality checks took seconds. Most time went to
  bibliographic drift, scope adjudication, anchor inventory, and proving the
  relationship between sources. That was genuinely necessary source work.
- The media-box/cropbox confusion and the non-`set -e` assertion episode were
  avoidable tooling friction.
