...
The slowest work was figure reconciliation. The supplied audit quickly proved
that emitted references and files agreed, but discovering figures absent from
both sides required reconstructing the printed number sequence, locating the
source pages, rendering them, and adjudicating four crops. This was genuinely
intricate, though the absence pattern was systematic once visible.

The next largest cost was structural reflow. Page boundaries, blank OCR blocks,
speaker turns, figures, duplicated overlap words, and running heads interact;
the work was repetitive because each rule needed a census and a controlled
renderer check. Apparatus classification was smaller but high-risk because a
translator note can contain mathematics and can cross a leaf.
