# Hildegard von Bingen, *Book of the Rewards of Life* — pipeline notes
...
Each checker was first run on its own known-bad fixture in `work/`:

- `lint-math.py` detected an unmatched `$` and exited 1;
- `check-math.js` rejected an undefined KaTeX command and exited 1; and
- `check-raw-latex.js` detected a backslash leaking into prose and exited 1.

All three then exited 0 on the final candidate. The candidate has zero dollar
signs, zero detected math blocks, and zero backslashes, so the clean triad says
only that prose will not break the math/Markdown consumer. It says nothing
about whether ABBYY transcribed any word correctly.
...
Recon, extraction, and consumer checks were fast. Most time went into
distinguishing page furniture from section headings and making paragraph joins
reviewable by source-shape counts. That work was genuinely structural because
ABBYY divided paragraphs into many text frames. The larger unresolved cost is
not tooling: correctness cannot progress without the missing printed-page
witness.
