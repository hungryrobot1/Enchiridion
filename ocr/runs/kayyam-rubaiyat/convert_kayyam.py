...
# The supplied PDF visibly prints this nonstandard label on p.18.  Preserve it
# as text, but do not promote it to a heading: heading promotion requires the
# validated numeral sequence.  The sibling PDF cannot establish whether the
# shared PG transcription agrees with an earlier paper edition.
...
def parse_edition(body: etree._Element, title: str) -> tuple[list[str], list[str], dict[str, int]]:
    ...
    for el in elements:
        ...
        if tag == "pre":
            ...
            out.append(f"## {label}" if sequence_valid else label)
            out.append("  \n".join(lines))
            source_stanzas.append(raw)
            pending_number = None
            continue
...
def convert(epub: Path, pdf: Path) -> tuple[str, dict[str, dict[str, int]]]:
    ...
    assert text.count("\n## ") == 75  # 74 valid numerals + intertitle
