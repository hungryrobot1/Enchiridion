...
- `convert_maimonides_epub.py` performs the only textual transformation. It
  selects the asserted source divisions, removes source page furniture and
  broken in-page navigation, flattens all links to text, preserves italics and
  adopted correction-span readings, and creates the reader hierarchy: document
  title as the opening `h1`, authorial introduction and Parts I–III as later
  `h1`s, chapters/subheads beneath them.
- The converter refuses changed chunk inventories, new note/figure/list/inline
  structures, altered section boundaries, altered chapter sequences, or token
  loss. It asserted 3 parts, chapter sequences 1–76 / 1–48 / 1–54 (178 total),
  970 non-furniture paragraphs, 1,900 italic elements, 93 correction spans, and
  581 source page/navigation markers. Output is 1,375,926 characters and 242,251
  whitespace-delimited words.
- `audit_maimonides.py` regenerates the Markdown in memory and requires byte-for-
  byte equality. It also verifies title-page/metadata facts and compares the
  full visible EPUB span with PDF pages 44–386 after removing only Calibre's
  generated first-line page labels.
- No text was edited by hand and no `toc.json` was created.
...
The actual conversion and all automated checks ran in seconds. Most time went
into mapping the EPUB's semantic divisions to the apparatus policy and proving
what the PDF could and could not witness. That is genuinely evidentiary work,
not computational cost.

The source-native stage still has no general conversion tool, so a fresh
assertion-heavy converter was required even though this EPUB uses unusually
regular TEI-derived XHTML. That is reasonable for a first shape, but it is the
main tooling cost if this PG TEI form recurs.

The triad summaries say “0 file(s)” when they mean zero files *with findings*,
and `check-math.js` says zero math blocks scanned. On a prose work this can look
like the target was not processed at all; the positive controls and explicit
zero-math interpretation are necessary to make the result legible.

The documentation was sufficient to decide the apparatus here and correctly
warned that source-native EPUB/PDF agreement is fidelity rather than
correctness. The remaining premise failure is at proofreading: the supplied PDF
is not a printed-page scan, and the proofreading workflow cannot cite pages it
does not have.
