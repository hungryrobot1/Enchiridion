#!/usr/bin/env python3
"""
Split a PDF to a specific page range and place the result in the text's home directory.
...
the text directory as <source_stem>-split.pdf. You can then run:

  python ocr.py texts/<era>/<text_id>/<source_stem>-split.pdf

and the OCR script will correctly derive the text_id from the parent
...
def find_pdf(text_dir: Path) -> Path:
    ...
    pdfs = [p for p in text_dir.glob("*.pdf") if not p.stem.endswith("-split")]
    if len(pdfs) == 0:
        raise FileNotFoundError(f"No PDF found in {text_dir}")
    if len(pdfs) > 1:
...
def resolve_source(arg: str) -> tuple[Path, Path]:
    ...
    text directory from its parent. Otherwise treat arg as a text_id and
    search texts/*/<text_id>/ for the PDF.
    """
    candidate = Path(arg)
...
def split_pdf(arg: str, start: int | None = None, end: int | None = None):
    ...
    output_pdf = text_dir / f"{source_pdf.stem}-split.pdf"
