...
"But the number which has none of these characteristics, but merely has in it an indeterminate multitude of units (πλήθος μονάδων ἄδριστον) is called ἀριθμός, 'number,' and its sign is s [= x]."

"And there is also another sign denoting that which is invariable in determinate numbers, namely the unit, the sign being M with o superposed, thus M̄."
...
¹ The literal rendering would be "A wanting multiplied by a wanting makes a forthcoming." The word corresponding to minus is λείψις ("wanting"): when it is used exactly as our minus is, it is in the dative λείψει, but there is some doubt whether Diophantus himself used this form (cf. p. 44 above). For the probable explanation of the sign, see pp. 42-44. The word for "forthcoming" is ὕπαρξις, from ὑπάρχω, to exist. Negative terms are λείποντα εἴδη, and positive ὑπάρχοντα.
...
Lesser number x. Therefore the larger is 3x + 4, and 4x + 4 = 80, so that x = 19.
...
Numbers 5x, x. Therefore 4x = 20, x = 5, and the numbers are 25, 5.
...
First given number 100, given fractions 1/3 and 1/6, given sum of fractions 30.
...
Given number 100, given fractions 1/4 and 1/6 respectively, given excess 20.

Second part 6x. Therefore the first part is 4 (x + 20).

Hence 10x + 80 = 100, x = 2, and the parts are 88, 12.
...
¹ Tannery attributes the alternative solution of 1. 18 (as of 1. 19) to an old scholiast.
...
Given number 100; and let (1) + (2) = 3. (3) and (2) + (3)

= 4. (1).

¹ Tannery attributes the alternative solution of 1. 19 (as of 1. 18) to an old scholiast.
...
Let $y$ be the first$^1$. Therefore sum of second and third $= 4y$, $5y = 100$ and $y = 20$.
...
Necessary condition. The middle number must exceed the least by such a fraction of the greatest that, if its denominator$^2$ be multiplied into the excess of the middle number over the least, the coefficient of $x$ in the product is greater than the coefficient of $x$ in the expression for the middle number resulting from the assumptions made$^3$.
...
$^1$ As already remarked (p. 52), Diophantus does not use a second symbol for the second unknown, but makes ἀριθμός do duty for the second as well as for the first.
...
$^3$ Wertheim points out that this condition has reference, not to the general solution of the problem, but to the general applicability of the particular procedure which Diophantus adopts in his solution. Suppose $X, Y, Z$ required such that $X - Y = Z/m$, $Y - Z = X/n$, $Z - a = Y/p$. Diophantus assumes $Z = x + a$, whence $Y = px$, $X = n(px - x - a)$. The condition states that $np - n > p$. If we solve for $x$ by substituting the values of $X, Y, Z$ in the first equation, we in fact obtain

$$m \{(np - n - p)x - na\} = x + a,$$

or $$x(mnp - mn - mp - 1) = a(mn + 1).$$
...
3. To find two numbers such that their product is to their sum or their difference in a given ratio [cf. 1. 34].
4. To find two numbers such that the sum of their squares is to their difference in a given ratio [cf. 1. 32].
5. To find two numbers such that the difference of their squares is to their sum in a given ratio [cf. 1. 33].
...
Take a square of the form$^1$ $(mx - 4)^2$, $m$ being any integer and 4 the number which is the square root of 16, e.g. take $(2x - 4)^2$, and equate it to $16 - x^2$.
...
9. To divide a given number which is the sum of two squares into two other squares$^2$.
...
$^1$ Diophantus' words are: "I form the square from any number of ἀφθῶσις minus as many units as there are in the side of 16." It is implied throughout that $m$ must be so chosen that the result may be rational in Diophantus' sense, i.e. rational and positive.

$^2$ Diophantus' solution is substantially the same as Euler's (Algebra, tr. Hewlett, Part II. Art. 219), though the latter is expressed more generally.

Required to find $x, y$ such that

$$x^2 + y^2 = f^2 + g^2.$$

If $x \gtrsim f$, then $y \lesssim g$.
...
This is called a double-equation (διπλοϊσότης).

To solve it, take the difference between the two expressions and resolve it into two factors$^1$; in this case let us say 4, $\frac{1}{4}$.
...
hence $2fpz + p^2z^2 - 2gqz + q^2z^2 = 0$,

and $z = \frac{2gq - 2fp}{p^2 + q^2}$,

so that $x = \frac{2gpq + f(q^2 - p^2)}{p^2 + q^2}$, $y = \frac{2fpq + g(p^2 - q^2)}{p^2 + q^2}$,
...
[Diophantus does not reduce to lowest terms, but says $x = \frac{4}{5}$ and then subtracts $\frac{16}{64}$ from 9 or $\frac{576}{64}$.]
...
$$\left. \begin{array}{l} x + 4 = u^2 \\ x + 7 = v^2 \end{array} \right\}$$

Suppose $x + 4 = p^2$;

therefore $x = p^2 - 4$, and $x + 7 = p^2 + 3$.

Suppose that $p^2 + 3 = (p + q)^2$;

therefore $p = (3 - q^2)/2q$.

Thus $x = (9 - 22q^2 + q^4)/4q^2$,
...
Take two numbers$^1$ such that the sum of their squares $< 20$, say 2, 3.
...
$$\begin{array}{l} x - y = (u^2 - v^2) = 2z(m - n) + m^2 - n^2 \\ = \frac{(m - n)(a + 2mn)}{m + n}. \end{array}$$
...
and, if $\left. \begin{array}{c} 4x + 4 \\ 6x + 9 \end{array} \right\}$ are respectively subtracted, the remainders are the same square.

Let then $x^2$ be the required square, and we have only to make $\left. \begin{array}{c} 4x + 4 \\ 6x + 9 \end{array} \right\}$ the required parts of 20.
...
Take $(x + m)^2$ for the required square$^1$, where $m^2$ is not greater than 20,

e.g. take $(x + 2)^2$.

This leaves a square if either $\left. \begin{array}{l} 4x + 4 \\ \text{or} \quad 2x + 3 \end{array} \right\}$ is subtracted.

Let these then be the parts of 20.

$^1$ Here again the implied condition, namely that $m^2$ is not greater than 20, is not necessary; the condition necessary for a real solution is something different.

The equations to be solved are $x + y = a$, $z^2 - x = u^2$, $z^2 - y = v^2$.

Diophantus here puts $(\xi + m)^2$ for $z^2$, so that, if $x = 2m\xi + m^2$, the second equation is satisfied. Now $(\xi + m)^2 - y$ must also be a square, and if this square is equal to $(\xi + m - n)^2$, say, we must have
...
Therefore $6x + 7 = 20$, and $x = \frac{18}{6}$.
...
¹ Tannery is of opinion that the problems 11. 17 and 18 have crept into the text from an ancient commentary to Book 1. to which they would more appropriately belong. Cf. 1. 22, 23.
...
Let first give to second $\frac{1}{8}$ of itself + 6, second to third $\frac{1}{8}$ of itself + 7, and third to first $\frac{1}{7}$ of itself + 8.
...
Therefore the greatest = $x^2 + 8x + 4 = \text{square} = (x + 3)^2$, say.
...
In fact $x = (26y - 18)/19$ and $z = (17y - 3)/19$,
...
$$y(5 \cdot 26 + 6 \cdot 19 + 7 \cdot 17) - 5 \cdot 18 - 7 \cdot 3 = 80 \cdot 19, y = \frac{1631}{363};$$
...
¹ Diophantus does not use two unknowns, but assumes the numbers to be x² and 1 until he has found x. Then he uses the same unknown (x) to find what he had first taken to be unity, as explained above, p. 52. The same remark applies to the next problem.
...
[Diophantus says $\frac{4}{6}$, and $\frac{23}{6}$, $\frac{8}{6}$, $\frac{2}{6}$.]
...
$$\left. \begin{array}{l} -x + y + z = \text{a square} \\ x - y + z = \text{a square} \\ x + y - z = \text{a square} \end{array} \right\},$$

and also

$$x + y + z = \text{a square}.$$

We have only to equate the first three expressions to squares $a^2, b^2, c^2$ such that $a^2 + b^2 + c^2 =$ a square, $k^2$ say, since the sum of the first three expressions is itself $x + y + z$.
...
sum of the three = x² + 4x + 7.

Therefore third = 4x + 4, second = x² - 2x, first = 2x + 3.
...
The third condition gives 52x² + 12 = a square; now 52 = 4.13, and 13 is not a square; but, if it were a square, the equation could easily be solved¹.
...
¹ The equation 52x² + 12 = u² can in reality be solved as it stands, by virtue of the fact that it has one obvious solution, namely x = 1. Another solution is found by substituting y + 1 for x, and so on. Cf. pp. 69, 70 above. The value x = 1 itself gives (13, 1, 4) as a solution of the problem.

² The method is indicated in II. 34. We have to find two pairs of squares differing by 12. (a) If we put 12 = 6.2, we have

$$\left\{\frac{1}{2}(6 - 2)\right\}^2 + 12 = \left\{\frac{1}{2}(6 + 2)\right\}^2,$$

and 16, 4 are squares differing by 12, or 4 is a square which when added to 12 gives a square. (b) If we put 12 = 4.3, we find $$\left\{\frac{1}{2}(4 - 3)\right\}^2$$ or $$\frac{1}{4}$$ to be a square which when added to 12 gives a square.
...
By the third condition, $266x^2 - 10$ must be a square; but 266 is not a square$^2$.

Therefore, as in the preceding problem, we must find two squares each of which exceeds a square by 10.

The squares $30\frac{1}{4}$, $12\frac{1}{4}$ satisfy these conditions$^3$.
...
$^1$ Euler (Algebra, Part II. Art. 232) has an elegant solution of this problem in whole numbers. Let it be required to find $x, y, z$ such that $xy + a, yz + a, zx + a$ are all squares. Suppose $xy + a = p^2$, and make $z = x + y + q$;

therefore $xz + a = x^2 + xy + qx + a = x^2 + qx + p^2$, and $yz + a = xy + y^2 + qy + a = y^2 + qy + p^2$;
...
E.g. suppose $a = 12$ and $p^2 = 25$, so that $xy = 13$; let $x = 1$, $y = 13$, and we have $z = 14 + 10 = 24$ or 4, and $(1, 13, 4)$, $(1, 13, 24)$ are solutions.

$^2$ As a matter of fact, the equation $266x^2 - 10 = u^2$ can be solved as it stands, since it has one obvious solution, namely $x = 1$. (Cf. pp. 69, 70 above and note on preceding problem, p. 159.) The value $x = 1$ gives $(14, 1, 19)$ as a solution of the problem.

$^3$ Tannery brackets the passage in the text in which these squares are found, on the ground that, as the solution was not given in the corresponding place of III. 10, there was no necessity to give it here. 10 and 1 being factors of 10,

$$\left\{\frac{1}{2}(10 - 1)\right\}^2 + 10 = \left\{\frac{1}{2}(10 + 1)\right\}^2;$$

thus $30\frac{1}{4}$ is a square which exceeds a square by 10. Similarly $\left\{\frac{1}{2}(5 + 2)\right\}^2$ or $12\frac{1}{4}$ is such a square. The latter is found in the text by putting $m^2 - 10 = \text{square} = (m - 2)^2$, whence $m = 3\frac{1}{4}$, and $m^2 = 12\frac{1}{4}$.
...
¹ Wertheim gives a more general solution, as follows. If we take as the required numbers $X = \frac{1}{4}ax$, $Y = ax + b^2$, $Z = \frac{1}{4}b^2$, two conditions are already satisfied, namely $XY + Z^2 = \text{a square}$, and $YZ + X^2 = \text{a square}$.

It only remains to satisfy the condition $ZX + Y^2 = \text{a square}$, or

$$a^2x^2 + \frac{33}{16}ab^2x + b^4 = \text{a square.}$$
...
$$\xi = \sqrt{\{(b^2 + 1)(c^2 + 1)/(a^2 + 1)\}},$$

with similar expressions for $\eta, \zeta$.

$x, y, z$ are these expressions minus 1 respectively. $a^2, b^2, c^2$ must of course be so chosen that the resulting values of $\xi, \eta, \zeta$ may be rational. Cf. Euler, Commentationes arithmeticae, II. p. 577.
...
$$\left. \begin{array}{l} yz + y + z = u^2 \\ zx + z + x = v^2 \\ xy + x + y = w^2 \end{array} \right\}$$

Fermat shows how to solve the corresponding problem with four numbers instead of three. He uses for this purpose Diophantus' solution of v. 5, namely the problem of finding x², y², z², such that

$$\left. \begin{array}{c} y^2z^2 + x^2 = r^2, \quad z^2x^2 + y^2 = s^2, \quad x^2y^2 + z^2 = t^2 \\ y^2z^2 + y^2 + z^2 = u^2, \quad z^2x^2 + z^2 + x^2 = v^2, \quad x^2y^2 + x^2 + y^2 = w^2 \end{array} \right\}$$
...
$$\left. \begin{array}{l} \frac{25}{9}x + x + \frac{25}{9}, \text{ or } \frac{34x}{9} + \frac{25}{9} \\ \frac{64}{9}x + x + \frac{64}{9}, \text{ or } \frac{73x}{9} + \frac{64}{9} \\ \frac{196}{9}x + x + \frac{196}{9}, \text{ or } \frac{205x}{9} + \frac{196}{9} \end{array} \right\}$$
...
$$\begin{array}{l} 34x + 25 = u^2, \\ 73x + 64 = v^2, \\ 205x + 196 = w^2. \end{array}$$

The method gives

$$x = - \frac{4598185984968447872000}{6316290048284196992001},$$

the denominator being equal to (25132230399)².

Verifying the correctness of the solution, we find that, in fact,

$$\begin{array}{l} \frac{34}{25}x + 1 = \left(\frac{2505136897}{25132230399}\right)^2, \\ \frac{73}{64}x + 1 = \left(\frac{10351251901}{25132230399}\right)^2, \\ \frac{205}{196}x + 1 = \left(\frac{12275841601}{25132230399}\right)^2. \end{array}$$
...
¹ With this problem should be compared that in paragraph 42 of Part I. of the *Inventum Novum* of Jacobus de Billy (*Oeuvres de Fermat*, III. pp. 351-2), where three conditions correspond to those of the above problem, and there is a fourth in addition. The problem is to find $$\xi, \eta$$ ($$\xi > \eta$$) such that

$$\left. \begin{array}{c} \xi - \xi\eta \\ \eta - \xi\eta \\ \xi + \eta - \xi\eta \\ \xi - \eta - \xi\eta \end{array} \right\}$$ are all squares.
...
¹ If there are two numbers p, q, to "form a right-angled triangle" from them means to take the numbers p² + q², p² - q², 2pq. These are the sides of a right-angled triangle, since

(p² + q²)² = (p² - q²)² + (2pq)².

² This problem and the next are the same as 11, 15, 14 respectively. It may therefore be doubted whether the solutions here given are genuine, especially as interpolations from ancient commentaries occur most at the beginning and end of Books,
...
so that $x^2 = (a - b^2)/3b$; $x$ is therefore "irrational" unless $(a - b^2)/3b$ is a square. In Diophantus' hypothesis $a$ is taken as 185, and $b$ as 5, and the condition is satisfied. He shows therefore incidentally that he knew how to find two numbers $a$, $b$ such that $(a - b^2)/3b$ is a square (Loria, Le scienze esatte nell'antica Grecia, Libro V. pp. 129-30).
...
$x^2 + y = \sqrt{(x + y)}$; therefore $x^4 + 2x^2y + y^2 = x + y$, or $y^2 - (1 - 2x^2)y = x - x^4$.

Solving for $y$, we obtain, as one of the solutions,

$$y = \frac{1}{2} - x^2 + \sqrt{\left(\frac{1}{4} + x - x^2\right)}.$$

To make the expression under the radical a square we put $\frac{1}{4} + x - x^2 = \left(mx - \frac{1}{2}\right)^2$, whence $x = \frac{m+1}{m^2+1}$, $y = \frac{m^4 + m^3 - m - 1}{(m^2+1)^2}$. Diophantus' solution corresponds to $m=2$.
...
That is, $8x^3 + x = 27x^3$, and $19x^3 = x$, or $19x^3 = 1$.

¹ Nesselmann (op. cit. p. 163) gives a more general solution.

We have $x^2 + y = (x + y)^2$, whence $1 = 3x^2 + 3xy + y^2$.

Solving for $y$, we find

$$y = -\frac{3}{2}x \pm \sqrt{\left(1 - \frac{3}{4}x^2\right)} = \frac{1}{2}\{-3x \pm \sqrt{(4 - 3x^2)}\}.$$

Lastly, putting $4 - 3x^2 = \left(2 - \frac{m}{n}x\right)^2$, we find $x = \frac{4mn}{3n^2 + m^2}$, $\sqrt{(4 - 3x^2)} = \pm \frac{2m^2 - 6n^2}{3n^2 + m^2}$, and $y = \frac{-6mn \pm (m^2 - 3n^2)}{3n^2 + m^2}$. If the positive sign be taken, then, in order that $y$ may always be positive, $m/n$ must be $> 3 + \sqrt{12}$; Diophantus' solution corresponds to $m = 7$, $n = 1$.

---

But 19 is not a square. Hence we must find, to replace it, some square number. Now $19x^8$ arises from $27x^3 - 8x^2$, where 27 is the cube of 3, and 8 the cube of 2. And the $3x$ comes from the assumed side $2x$, by increasing the coefficient by unity.

Thus we must find two consecutive numbers such that their cubes differ by a square.

Let them be $y, y + 1$.

Therefore $3y^2 + 3y + 1 = \text{square} = (1 - 2y)^2$, say, and $y = 7$.
...
Therefore $x^2 = \frac{1}{169}$, and $x = \frac{1}{13}$.
...
Therefore $35x^3 - 2x =$ side of cube $27x^3 = 3x$, or $35x^3 = 5$.
...
That is, $4 - 6z + 3z^2 = \text{square} = (2 - 4z)^2$, say; $z = \frac{10}{13}$, and the sides are $\frac{10}{13}, \frac{18}{13}$.
...
We have therefore, as in the last problem, to find two cubes the sum of which has to the sum of their sides the ratio of a square to a square$^1$.

These are found, as before, to be $5^2, 8^2$.
...
$^1$ Here, as in the last problem, Diophantus could have solved his auxiliary problem of making $(x^3 + y^3)/(x + y)$ a square by making $x^3 - xy + y^2$ a square in the same way as in Lemma 1. to v. 7 he makes $x^3 + xy + y^2$ a square.
...
$$\left. \begin{array}{c} x(1 + 2k) = y(1 - k^2) \\ x + ky = \pm 1 \end{array} \right\},$$

which give $$x = \pm \frac{1 - k^2}{1 + k + k^2}, \quad y = \pm \frac{1 + 2k}{1 + k + k^2},$$
...
Nesselmann's method of solution (cf. note on IV. 9) gives $$x = \frac{1}{2} \{-y \pm \sqrt{(4 - 3y^2)}\}$$, and hence $$y = \frac{4mn}{m^2 + 3n^2}$$, $$x = \frac{-2mn \pm (m^2 - 3n^2)}{m^2 + 3n^2}$$. Diophantus' solution is obtained by putting m = 1, n = 2 and taking the lower sign.
...
subject to the condition that m is either a square or the third part of a square. His method corresponds to that of Diophantus. He does not divide out by x ± y, and he reduces the problem to the subsidiary one of finding ξ, η such that the ratio of ξ³ ± η³ to ξ ± η is the ratio of a square to a square. His assumptions for the "sides," ξ, η, are of the same kind as those made by Diophantus; in the first problem he assumes x, 6 - x and in the second x, x + 2. In fact, it being given that (x³ ± y³)/(x ± y) = a, Bachet assumes x ± y = z and thus obtains
...
Let the least square be 1, the greatest x² + 2x + 1;
...
therefore twice difference of greatest and least = sum of the three = 2x² + 4x.

But least + greatest = x² + 2x + 2, so that

middle = x² + 2x - 2.

Hence x² + 2x - 2 = square = (x - 4)², say, and x = 1/4.
...
Now the sum of the three numbers = a square; therefore $13x =$ a square $= 169y^2$, say$^1$.

The numbers are then $13y^2 - 1$, $52y^2$, $104y^2 + 1$.

Lastly, (third)$^2 +$ first = a square.

Therefore $10816y^4 + 221y^2 =$ a square,

or $10816y^2 + 221 =$ a square $= (104y + 1)^2$, say.

Therefore $y = \frac{220}{208} = \frac{55}{52}$,
...
$$a = \frac{1}{2}(a - b + c), \quad \beta = \frac{1}{2}(-a + b + c).$$

Thus $$z = \sqrt{\left(\frac{a\beta}{a - a}\right)} = \sqrt{\left\{\frac{(a - b + c)(-a + b + c)}{2(a + b - c)}\right\}},$$
...
$^1$ Diophantus uses the same unknown $\epsilon$ for $y$ as for $x$, writing actually $\kappa\alpha\iota\gamma\nu\tau\alpha\iota b \iota \Delta^V\iota\gamma$, literally "and $x$ becomes $13x^2$."
...
¹ The expression is ἐν τῷ ἀορίστῳ, which is defined at the end of the problem to mean in terms of one unknown (and units), so that the conditions of the problem are satisfied whatever value is given to the unknown.
...
$xy + 1, yz + 1, zx + 1$ are all squares.

Suppose $zx + 1 = p^2, yz + 1 = q^2,$

so that $x = (p^2 - 1)/z, y = (q^2 - 1)/z.$

Therefore $xy + 1 = \frac{(p^2 - 1)(q^2 - 1)}{z^2} + 1 =$ a square,

or $(p^2 - 1)(q^2 - 1) + z^2 =$ a square

$$= (z - r)^2, \text{ say; } [\text{Euler has } (z + r)^2]$$

whence $z = \frac{r^2 - (p^2 - 1)(q^2 - 1)}{2r},$
...
$$z = \frac{(p + q)^2}{2(pq + 1)} \text{ and } x = \frac{2(pq + 1)(p^2 - 1)}{(p + q)^2}, \quad y = \frac{2(pq + 1)(q^2 - 1)}{(p + q)^2}.$$
...
and $\frac{1}{16}, \frac{33}{16}, \frac{68}{16}, \frac{105}{16}$ is the solution$^2$.

$^1$ The remaining condition was: product of second and third $+1 = \text{a square}$. That this is satisfied also follows from the general property stated in the last note. In fact

$$(x+2)(4x+4)+1=4x^2+12x+9,$$

which is a square.

$^2$ With this solution should be compared Euler's solution (Algebra, Part II. Art. 233) of the problem of finding $x, y, z, v$ such that the six expressions

$$xy+a, yz+a, zx+a, x\dot{v}+a, yv+a, zv+a$$
...
If we begin by supposing $xy+a=p^2$, and take $z=x+y+p$ $2p$, the second and third expressions become squares (vide note on III. 10, p. 160).
...
or (since $xy+a=p^2$) $x^2-2xy+y^2-3a=\text{a square}$.

Suppose that $(x-y)^2-3a=(x-y-q)^2$;

therefore $x-y=(q^2+3a)/2q$,

or $x=y+\frac{q^2+3a}{2q}$.

Consequently $p^2=xy+a=y^2+\frac{q^2+3a}{2q}y+a$.

If we put $p=y+r$, we have

$$2ry+r^2=\frac{q^2+3a}{2q}y+a,$$

and $y=\frac{2qr^2-2aq}{q^2-4qr+3a}$,
...
whence $$\frac{1}{4} v = (m + n + l) + mn (m + n + 2l)$$
...
³ Format gives a solution which avoids the necessity for the auxiliary problem.

Let the solid content be $x^2 - 2x$, the first number 1, and the second number $2x$; two conditions are thus satisfied.

The third number is now $x^2 - 2x$ divided by $2x \cdot 1$, or $\frac{1}{2}x - 1$; and the third condition gives

$$x^2 - \frac{3}{2}x - 1 = \text{a square}.$$

Now $x$ must be greater than 2; we therefore put

$$x^2 - \frac{3}{2}x - 1 = (x - m)^2,$$
...
(1) ξ - ξηξ, (4) η - ξ - ξηξ, (7) ξη - ξηζ,

(2) η - ξηζ, (5) ζ - ξ - ξηξ, (8) ηζ - ξηζ,

(3) ζ - ξηξ, (6) η - ζ - ξηξ, (9) η² - ξηζ.
...
And, by (5), 1 - 3x + x² = a square.

Solving this double-equation in the usual way, we get x = 3/8, and the numbers are 3/8, 1, 5/8.

Another solution can be obtained by putting y + 3/8 in place of x in the two expressions, and so on. Cf. note on III. 18 above.

It would appear from a letter from Fermat to De Billy of 26 Aug. 1659 (Oeuvres, II. pp. 436-8) that this problem and the above single solution were De Billy's own. De Billy had supposed that this was the only solution, but Fermat observed that there were any number, as the above double-equation has any number of solutions. Fermat gave (10416/51865, 1, 41449/51865) as another solution.
...
¹ Diophantus means that, if we are to get rid of the third power and the absolute term, we can only put the expression equal to $(2x - 1)^3$, which gives a negative and therefore "impossible" value for $x$. But the equation is not really impossible, for we can get rid of the terms in $x^2$ and $x^3$ by putting
...
Accordingly we assume for the first number an expression of the form $m^2x + 1$, say $8x + 1$, and for the second number $x^2$ (since $8x^2 + x^2 - x^2 = a$ cube).

Also $8x^2 + x^2 - 8x - 1 = a$ cube $= (2x - 1)^2$, say.
...
Therefore we have to find two numbers such that their sum is 28 and their product 36. If $14 + x$, $14 - x$ are the numbers$^1$, we have $196 - x^2 = 36$, or $x^2 = 160$; and, if 160 were a square, we should have a rational solution.
...
$\frac{1}{2}$ the sum of the cubes is $z^2 + 3z$;

therefore $2\frac{1}{4}z^4 + 1\frac{1}{2}z^2 + \frac{1}{4} - z^3 - 3z = a$ square,

or $9z^4 + 6z^2 + 1 - 4z^3 - 12z = a$ square $= (3z^2 + 1 - 6z)^2$, say;

whence $32z^3 = 36z^2$, and $z = \frac{9}{8}$.
...
Now let the first number $= x + \text{half sum} = x + \frac{1228}{512}$, and the second = half sum $- x = \frac{1228}{512} - x$;
...
$x^2 - x^2 - x^2 = x^3 - 2x^2 =$ a cube (less than $x^3$) $= (\frac{1}{2}x)^3$, say.
...
Unfortunately the promised separate work did not appear. The theorem so far as it relates to squares was first proved by Lagrange (Nouv. Mémoires de l'Acad. de Berlin, année 1770, Berlin 1772, pp. 123-133; Oeuvres, III. pp. 189-201), who followed up results obtained by Euler. Cf. also Legendre, Zahlentheorie, tr. Maser, I. pp. 212 sqq. Lagrange's proof is set out as shortly as possible in Wertheim's Diophantus, pp. 324-330. The theorem of Fermat in all its generality was proved by Cauchy (Oeuvres, IIe série, Vol. VI. pp. 320-353); cf. Legendre, Zahlentheorie, tr. Maser, II. pp. 332 sqq.

---

Now $13 = 4 + 9 = (\frac{64}{25} + \frac{96}{25}) + (\frac{144}{25} + \frac{81}{25})$,
...
Put now second part $= \frac{5}{3}$; therefore first $= \frac{12}{3} - x$.
...
1 Fermat observes: "The following is an easier method of solution. Divide the number 6 into two in any manner, e.g. into 5 and 1. Divide their product less 1, that is 4, by 6, the given number: the result is $\frac{2}{3}$. Subtracting this first from 5 and then from 1,

---

# BOOK IV

35. To find two numbers such that the first with a fraction of the second is to the remainder of the second in a given ratio, and also the second with the same fraction of the first is to the remainder of the first in a given ratio.
...
[The fraction may be either an aliquot part or not, τὸ αὐτὸ μέρος or τὰ αὐτὸ μέρη as Diophantus says, following the ordinary definition of these terms (the
...
¹ The Greek phrase is ἐὰν ἄρα τάξωμεν τὸν βοῦ σοῦ οἰουδήποτε (οἰουδήποτε s in Lemma to IV. 36), "If we make the second" (literally "put the second at") "any s whatever." But the s is not here, as it is in the Lemma to IV. 36, the actual x of the problem, for Diophantus goes on to say "E.g. let the second be x - 1." In the Lemma to IV. 34 the corresponding expression is "any quantity whatever" (ὀσουδήποτε without s). The present Lemma amounts to saying that, if $xy + x + y = a$, then $x = (a - y)/(y + 1)$.

² This determinate set of equations can of course be solved, with our notation, by a simple substitution.

The equations

$$\left. \begin{array}{l} yz + y + z = a \\ zx + z + x = b \\ xy + x + y = c \end{array} \right\}$$

are equivalent to $$\left\{ \begin{array}{l} (y + 1)(z + 1) = a + 1, \\ (z + 1)(x + 1) = b + 1, \\ (x + 1)(y + 1) = c + 1, \end{array} \right.$$ or $$\left\{ \begin{array}{l} \eta\zeta = a + 1, \\ \zeta\zeta = b + 1, \\ \zeta\eta = c + 1, \end{array} \right.$$

where $$\xi = x + 1, \eta = y + 1, \zeta = z + 1.$$

The solution is $$\xi = x + 1 = \sqrt{\left\{\frac{(b + 1)(c + 1)}{a + 1}\right\}}$$ etc.
